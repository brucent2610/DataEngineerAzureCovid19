...
STORAGE_ACCOUNT_CONTAINER_SCRIPTS_NAME=scripts
SOURCE_FILE_NAME=population_by_age.tsv.gz

DATALAKE_STORAGE_GEN2_CONTAINER_RAW_NAME=raw
DATALAKE_STORAGE_GEN2_CONTAINER_LOOKUP_NAME=lookup
DATALAKE_STORAGE_GEN2_CONTAINER_PROCESSED_NAME=processed
...
az storage container create --account-name $STORAGE_ACCOUNT_NAME --name $STORAGE_ACCOUNT_CONTAINER_SCRIPTS_NAME --auth-mode login
...
az storage container create --account-name $DATALAKE_STORAGE_GEN2_NAME --name $DATALAKE_STORAGE_GEN2_CONTAINER_RAW_NAME --auth-mode login
az storage container create --account-name $DATALAKE_STORAGE_GEN2_NAME --name $DATALAKE_STORAGE_GEN2_CONTAINER_LOOKUP_NAME --auth-mode login
az storage container create --account-name $DATALAKE_STORAGE_GEN2_NAME --name $DATALAKE_STORAGE_GEN2_CONTAINER_PROCESSED_NAME --auth-mode login
